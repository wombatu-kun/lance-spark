...
public class LanceStreamingWrite implements StreamingWrite {
  ...
  public LanceStreamingWrite(
    ...
    try (Dataset probe = Utils.openDatasetBuilder(writeOptions).build()) {
   ...
   * Utils#openDatasetBuilder} and to a versioned-open path that threads a {@code version} through
   * {@link ReadOptions.Builder#setVersion(int)}. Exposed as package-private for test visibility.
   */
  StreamingCommitProtocol buildProtocol() {
    StreamingCommitProtocol.DatasetOpener opener =
        () -> Utils.openDatasetBuilder(writeOptions).build();
